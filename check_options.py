...
def fmt_row(p):
    ...
    if status == "CLOSED":
        # FIX: option_positions rows use the column `exit_premium` (set by
        # DatabaseManager.close_option_position()), not `exit_price` or
        # `current_price` - those two belong to the equity `positions`
        # table only and are never populated on an option row. The old
        # lookup always fell through to "?" here even though the DB (and
        # the P&L math, which is correct) had the real value all along.
        lines.append(f"  Exit premium: {p.get('exit_premium', '?')}   Exit time: {p.get('exit_time', '?')}")
